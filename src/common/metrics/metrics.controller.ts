...
export class MetricsController {
  ...
  getMetricsSummary() {
    ...
    return {
      ...
      auth: {
        totalLoginAttempts: this.metricsService.getCounter(
          'auth_events_total',
          { event: 'login' },
        ),
        successfulLogins: this.metricsService.getCounter('auth_events_total', {
          event: 'login',
          success: 'true',
        }),
        failedLogins: this.metricsService.getCounter('auth_events_total', {
          event: 'login',
          success: 'false',
        }),
      },
      database: {
        totalOperations: this.metricsService.getCounter(
      ...
      files: {
        ...
        uploads: this.metricsService.getCounter('file_operations_total', {
          operation: 'upload',
        }),
        downloads: this.metricsService.getCounter('file_operations_total', {
          operation: 'download',
        }),
      },
  ...
  private calculateAverageResponseTime(): number {
    const histogram = this.metricsService.getHistogram(
